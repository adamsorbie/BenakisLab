...
##### Example of how to load and process data

# read in predicted metagenome
picrust_ko <- read_tsv(gzfile("pred_metagenome_unstrat.tsv.gz"))
# read in metadata
meta <- read_tsv("metadata.tsv")

# this tranposes the data so samples are rows and adds the metadata variables
picrust_meta <- picrust_ko %>% 
  as.matrix() %>% 
  t() %>% 
  #' change to metadata var you're interested in e.g. temperature and add as many 
  #' or as few as you like
  merge(meta_tumor[c("Var1", "Var2")],by=0) %>%  
